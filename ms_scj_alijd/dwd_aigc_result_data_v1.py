...
result_table = '{{ dwd_aigc_result_data }}'
...
def transform(row):
    # 标准品牌名清洗 - 不可为空 用规则集104982进行修正（SQL已完成第一步修正）
    ...
    # 2. 品类规则优先级2 (规则集104981) - 使用104997清洗后的std_category_1,2和原始std_category_3,4作为输入
    ...
    for i in range(1, 5):  # std_category_1, std_category_2, std_category_3, std_category_4
        key = f'std_category_{i}'
        if key in value2 and value2[key] not in [None, 'NULL', '']:
            row[key] = value2[key]
